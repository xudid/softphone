...
public class SoftPhone {
	...
	public SoftPhone(final SipAccount account, UserAgentClient userAgentClient, UserAgentServer userAgentServer,
			ContactBook book, MediaController mc, SdpProcessor sp, ConfigurationProxy configurationProxy) {
		...
		UserAgentListener ual = new UserAgentListener() {
			...
			public void processIncommingCallEvent(IncommingCallEvent event) {
				...
				if (online) {
					...
					online = true;
					sipAddress = ((ContactHeader) request.getHeader(ContactHeader.NAME)).getAddress().toString();
					String sessionID = event.getTransactionID();
					CallParticipantInterface remoteCallParticipant = new CallParticipant(sipAddress);
					int contentLength = ((ContentLengthHeader) request.getHeader(ContentLengthHeader.NAME))
							.getContentLength();

					if (contentLength > 0) {
						String cType = ((ContentTypeHeader) request.getHeader(ContentTypeHeader.NAME)).getContentType();
						String subType = ((ContentTypeHeader) request.getHeader(ContentTypeHeader.NAME))
								.getContentSubType();
						...
						if (b.toString().equalsIgnoreCase("application/sdp")) {
							...
							currentCall = new CallSession(account, remoteCallParticipant, offer);
							currentCall.setIncommingCallRequest(request);
							currentCall.setCallSessionID(sessionID);
							String sdpresponse = sdpProcessor.makeSdpResponse(account.getDisplayName(), offer);
							System.out.println(sdpresponse);
							currentCall.setSdpResponse(sdpresponse);
							RTPSessionConfiguration config = sdpProcessor.getIncommingCallRTPConf(offer, sdpresponse);
...
//                                                 );
			...
			public void processOutGoingCallEvent(OutGoingCallAcceptedEvent ev) {
				try {
					...
					currentCall.setSdpResponse(sdpResponse);
					CallParticipantInterface c = currentCall.getDistantParticipant();
					String addr = sdpProcessor.makeSessionDescription(sdpResponse).getConnection().getAddress();
					InetAddress IPAddress = NetworkUtils.StringToInet4Address(addr);
					c.setIPAddress(IPAddress);
					RTPSessionConfiguration config = sdpProcessor.processSdpResponse(ev.getSdpResponse(),
							currentCall.getSdpOffer());
					mediaController.startSendingMedia(config);
					fireCallEstablished();

				} catch (SdpParseException ex) {
					Logger.getLogger(SoftPhone.class.getName()).log(Level.SEVERE, null, ex);
				}
			...
			/*
			 * Traite l'événement de début de session ,garde une référence de la session
			 */
			...
			/*
			 * Traite les réponses 401 et 407 pour les appels sortants et les enregistrement
			 */
			public void processWWWAuthRequired(wwwAuthRequired event) {
				...
				if (method.equalsIgnoreCase(Request.REGISTER)) {
					...
					Register(currentRegisterSequence, event.getCseqNum(), event.getResponse());
	...
	/*
	 * Continue une session d'appel suite à une demande d'authentification
	 * 
	...
	/*
	 * Met fin à l'appel en cours par une requete Cancel si l'Invite n'a pas reçu de
	 * réponse finale par une requete Bye dans le cas contraire
	 */
	...
	/*
	 * Programme le prochaine enregistrement Averti l'utilisateur que le SoftPhone
	 * est enregistré auprès du serveur de localisation deplacer la logique de
	 * trigger dans userAgentRegister et ne laisser ici que le déclenchement d'un
	 * évènement vers l'ui
	 */
